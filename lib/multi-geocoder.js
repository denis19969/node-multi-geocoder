    ...
    return vow.allResolved(tasks)
      .then(function (results) {
        ...
        results.forEach(function (promise, index) {
          ...
          if (promise.isFulfilled() && options && options.unique) {
            const duplicate = features
              .find(point => point.properties.metaDataProperty.GeocoderMetaData.text === value.properties.metaDataProperty.GeocoderMetaData.text);
            if (duplicate) {
              errors.push({
                request: points[index],
                index: points[index].index,
                reason: `A point with this address already exists (${duplicate.request.name || ''})`
              });
            } else {
              features.push({request: points[index], ...value, index: points[index].index});
            }
