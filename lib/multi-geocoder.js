...
module.exports = inherit(/** @lends Geocoder.prototype */ {
    ...
    geocode: function (points, options) {
        ...
        if (options && options.unique) {
            points.forEach(el => valid.find(point => point.address === el.address) ?
              errors.push({request: el, index: el.index, reason: 'Point already exists'}) : valid.push(el));
        } else {
            valid = points;
        }
